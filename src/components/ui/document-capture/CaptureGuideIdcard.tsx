...
interface CaptureGuideIdcardProps {
  title?: string
  onStart: () => void
  onClose: () => void
}

export function CaptureGuideIdcard({ title = "신분증", onStart, onClose }: CaptureGuideIdcardProps) {
  const steps = [
  ...
  return (
    <div className="fixed inset-0 z-50 flex h-screen flex-col bg-white">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6">
        ...
        <h1 className="text-xl font-bold text-slate-900 mb-2">{title} 촬영</h1>
        <p className="text-sm text-gray-500 mb-8">본인의 {title}을 준비해주세요.</p>
