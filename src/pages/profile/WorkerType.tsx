...
export function WorkerTypePage({ mode = "profile" }: WorkerTypePageProps) {
  ...
  // 여권(외국인 미등록) 가입자 = 연수생. 가입 시 workerCategory 를 안 받으므로
  // nationalityType 으로 판별 (이미 trainee 로 저장된 워커도 포함).
  const isTraineeWorker =
    profile?.workerCategory?.toLowerCase() === "trainee" ||
        ...
        {isLoading ? (
          ...
          <div className="px-4 space-y-3">
            {(workerTypes ?? []).filter((t) => {
              ...
              // 연수생 워커는 연수생 카드만, 그 외 워커는 연수생 카드를 숨김.
              return isTraineeWorker ? code === "trainee" : code !== "trainee"
            }).map((type) => {
