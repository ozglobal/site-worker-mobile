...
export function ListPage() {
  ...
  const records: WeeklyAttendanceRecord[] = useMemo(() => {
    // entryId 는 monthly 응답이 entry 단위로 직접 제공 → 분할(공수분할) entry 의
    // 정정요청을 entry 단위로 정확히 매핑. attendanceId 는 monthly 응답에 없으므로
    // 정정요청 제출(오늘 분만 가능)용으로 todayDaily 에서 siteId 기준으로 보강.
    const todayAttendanceBySite = new Map<string, string>()
    ;(todayDaily?.attendances || []).forEach((a) => {
      if (a.siteId && a.attendanceId && !todayAttendanceBySite.has(a.siteId)) {
        todayAttendanceBySite.set(a.siteId, a.attendanceId)
      }
    })
    ...
    ;(data?.days || []).forEach((d) => {
      d.entries?.forEach((e, idx) => {
        ...
        const attendanceId = e.attendanceId ?? (isToday ? todayAttendanceBySite.get(e.siteId) : undefined)
        fromDays.push({
