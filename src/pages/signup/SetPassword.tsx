...
export function SetPasswordPage() {
  ...
  const handleSave = async () => {
    ...
    // 회원 유형(workerCategory)은 원칙적으로 가입 시 보내지 않고 로그인 후 온보딩에서 설정.
    // 단, 미등록 외국인(여권 가입)은 백엔드 제약상 반드시 trainee 여야 하므로 가입 시 함께 전송.
    // (WorkerCategoryConstraintValidator: foreigner_unregistered ⟺ trainee)
    const result = await registerWorker({
      ...
      ...(data.nationalityType === 'foreigner_unregistered' ? { workerCategory: 'trainee' } : {}),
