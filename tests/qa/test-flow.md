...
## 시나리오 3B — 휴대폰 번호 변경 플로우

```mermaid
flowchart TD
    A(["/profile/myinfo-rrn\n또는 myinfo-frn\n또는 myinfo-pn"]) --> B["수정 버튼 표시 확인"]:::check
    B --> C["수정 탭"]
    C --> D["휴대폰 번호 변경 모달 열림 확인"]:::check

    D --> E["새 휴대폰 번호 입력"]
    E --> F{11자리?}
    F -->|미만| G["인증번호 받기 비활성 확인\n인증번호 입력란 숨김 확인"]:::check
    F -->|완성| H["인증번호 받기 활성 확인"]:::check
    H --> I["인증번호 받기 탭"]
    I --> J{SMS 수신}
    J -->|발송 성공| K["인증번호가 발송되었습니다 토스트\n인증번호 입력란 표시"]:::check
    J -->|API 실패| ERR["오류 토스트\n모달 유지"]:::fail
    K --> L["6자리 코드 입력"]
    L --> M{6자리?}
    M -->|미만| N["변경하기 비활성 확인"]:::check
    M -->|완성| O["변경하기 활성 확인"]:::check
    O --> P["변경하기 탭"]
    P --> Q{API 응답}
    Q -->|성공| R["전화번호가 xxx 으로 변경되었습니다\n다시 로그인 하시겠습니까? 다이얼로그"]:::pass
    Q -->|실패| S["오류 토스트\n모달 유지"]:::fail
    R --> T{재로그인?}
    T -->|예| U["로그아웃 → 로그인 화면"]:::pass
    T -->|아니오| V["다이얼로그 닫힘"]:::pass
...
![시나리오 3B — 휴대폰 번호 변경](./images/test-3b-phone-change.png)
...
```mermaid
flowchart TD
    A([당일 출퇴근 상세\n또는 홈 화면]) --> B["정정 요청 버튼 탭"]
    B --> C["정정요청 다이얼로그 열림\n현장명·시간 미리 채워짐 확인"]:::check

    C --> D["야근했어요 체크박스 확인\n이미 야근 기록 있으면 비활성"]:::check
    C --> E["적용단가 현재 값 표시\n새 값 입력란 표시 확인"]:::check
    C --> F["사유 입력란 표시 확인"]:::check

    D & E & F --> G{사유 비어있음?}
    G -->|예| H["요청 제출하기 비활성 확인"]:::check
    G -->|아니오| I["요청 제출하기 활성"]:::check

    I --> J["요청 제출하기 탭"]
    J --> K{API 응답}
    K -->|성공| L["정정 요청이 제출되었습니다 토스트\n다이얼로그 닫힘 확인"]:::pass
    K -->|실패| M["오류 토스트\n화면 유지 + 재시도 가능"]:::fail
